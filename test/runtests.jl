...
@testset "limacon_polarsegment.jl" begin
    ...
    s_of_t_cs,t_of_s_cs=construct_arc_length_interpolation(Float64,crv;method=:cubic_spline,n_samples=2000)
...
@testset "symmetry rotations" begin
    ...
    dx=pt[1]-c0[1]
    dy=pt[2]-c0[2]
    @test pt90≈SVector{2,T}(c0[1]-dy,c0[2]+dx) atol=1e-10
    # normals rotate about origin (no c0 shift)
    n90=apply_symmetry_normals(rot90,n)
    @test n90≈SVector{2,T}(-n[2],n[1]) atol=1e-10
    ...
    @test norm(apply_symmetry_normals(rot1,n))≈norm(n) atol=1e-10
end
...
@testset "Cn_symmetry generator" begin
    ...
    @test apply_symmetry(syms[1],pt)≈pt atol=1e-10
