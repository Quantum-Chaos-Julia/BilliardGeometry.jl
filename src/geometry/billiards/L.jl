...
"""
    LShapeBilliard(a::T) where {T<:Real} → LShapeBilliard{T}

Construct the MATLAB-logo L-shaped billiard of outer half-size `a`,
with vertices
...
The full physical boundary is traversed clockwise as

    A -> F -> E -> D -> C -> B -> A.

## Arguments
* `a::T`: Half-size of the parent square.

## Returns
* `billiard::LShapeBilliard{T}`: L-shaped billiard with diagonal symmetry.
"""
function LShapeBilliard(a::T) where {T<:Real}
    ...
    # Full physical L boundary. Start at the reentrant corner and traverse
    # counterclockwise around the physical domain.
    ...
    # One diagonal half, y≤x. The diagonal intersects the outer boundary at
    # D=(-a,-a) and terminates at the reentrant corner A=(0,0).
    # A -> B -> C -> D are physical boundary pieces.
    # D -> A is the diagonal symmetry wall.
    f1=BilliardGeometry.LineSegment(A,B;bc=bc,domain_id=1,segment_id=1)
    f2=BilliardGeometry.LineSegment(B,C;bc=bc,domain_id=1,segment_id=2)
    f3=BilliardGeometry.LineSegment(C,D;bc=bc,domain_id=1,segment_id=3)
    f4=BilliardGeometry.LineSegment(D,A;bc=BilliardGeometry.ReflectionSymmetry(BilliardGeometry.DiagonalReflection(),-1),domain_id=1,segment_id=4)
    fundamental_boundary=BilliardGeometry.AbsCurve[f1,f2,f3,f4]
    vertices=SVector{2,T}[A,B,C,D]
