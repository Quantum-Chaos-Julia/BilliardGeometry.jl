...
#curves live in one MultiplyConnectedDomain (not a CompositeDomain: is_inside
#for a CompositeDomain is a union over subdomains, but a domain-with-a-hole
#needs the intersection "inside outer AND outside inner" that a single
#MultiplyConnectedDomain's all(...)-over-curves already gives): an outer full
#CircleSegment (orientation=1, physical wall) and an inner full CircleSegment
